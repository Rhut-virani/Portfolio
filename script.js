...
const devicenumber = $(window).innerWidth() + $(window).innerHeight(),
    ...
    $ptRight = $('.p-t-Right'),
    $allcontainer = $(
        '.projectContainer, .contactContainer, .skillsContainer, .aboutContainer',
    ),
    better =		$(window).width() < $(window).height()
		    ? $(window).width()
		    : $(window).height(),
...
$(document).ready(() => {
    ...
    $('.buttonContainer').on('click', () => {
        ...
        $("a.floatingText:contains('P')")
            .first()
            .attr('class', 'floatingText floatingmainText gotoLeft');
        $("a.floatingText:contains('r')")
            .first()
            .attr('class', 'floatingText floatingmainText gotoLeft');
        $("a.floatingText:contains('o')")
            .first()
            .attr('class', 'floatingText floatingmainText gotoLeft');
        $("a.floatingText:contains('j')")
            .first()
            .attr('class', 'floatingText floatingmainText gotoLeft');
        $("a.floatingText:contains('e')")
            .first()
            .attr('class', 'floatingText floatingmainText gotoLeft');
        $("a.floatingText:contains('c')")
            .first()
            .attr('class', 'floatingText floatingmainText gotoLeft');
        $("a.floatingText:contains('t')")
            .first()
            .attr('class', 'floatingText floatingmainText gotoLeft');
        $("a.floatingText:contains('s')")
            .first()
            .attr('class', 'floatingText floatingmainText gotoLeft');
        $("a.floatingText:contains('C')")
            .eq(1)
            .attr('class', 'floatingText floatingmainText gotoRight');
        $("a.floatingText:contains('o')")
            .eq(1)
            .attr('class', 'floatingText floatingmainText gotoRight');
        $("a.floatingText:contains('n')")
            .first()
            .attr('class', 'floatingText floatingmainText gotoRight');
        $("a.floatingText:contains('t')")
            .eq(1)
            .attr('class', 'floatingText floatingmainText gotoRight');
        $("a.floatingText:contains('a')")
            .first()
            .attr('class', 'floatingText floatingmainText gotoRight');
        $("a.floatingText:contains('c')")
            .eq(2)
        ...
        $("a.floatingText:contains('a')")
            .eq(1)
            .attr('class', 'floatingText floatingmainText gotodown');
        $("a.floatingText:contains('b')")
            .eq(1)
            .attr('class', 'floatingText floatingmainText gotodown');
        $("a.floatingText:contains('o')")
            .eq(2)
            .attr('class', 'floatingText floatingmainText gotodown');
        $("a.floatingText:contains('u')")
            .first()
            .attr('class', 'floatingText floatingmainText gotodown');
        $("a.floatingText:contains('t')")
            .eq(3)
        ...
        $("a.floatingText:contains('s')")
            .eq(1)
            .attr('class', 'floatingText floatingmainText gotoup');
        $("a.floatingText:contains('k')")
            .first()
            .attr('class', 'floatingText floatingmainText gotoup');
        $("a.floatingText:contains('i')")
            .first()
            .attr('class', 'floatingText floatingmainText gotoup');
        $("a.floatingText:contains('l')")
            .first()
            .attr('class', 'floatingText floatingmainText gotoup');
        $("a.floatingText:contains('l')")
            .eq(1)
    ...
    const tp = new TimelineMax({ paused: true })
        ...
        .to(
            ...
            0.5,
            { boxShadow: '-5px 0px 25px -15px #ffd000' },
            '#pause',
        );
    ...
    function scroll(
        ...
        const tl = new TimelineMax({ paused: true })
            ...
            .to(
                indicatorIn,
                0.1,
                { className: '+=active', color: '#ffd000', scale: 1.2 },
                '-=0.25',
            )
    ...
    scrolling = (e) => {
        ...
        if (e.originalEvent.deltaY > 0) {
            const sectionIn =					$('.home.active').next($home).length === 0
                ...
                indicatorIn =					$('.indication.active').next($indi).length === 0
            ...
            const sectionIn =					$('.home.active').prev($home).length === 0
					    ? $('.home.active').nextAll($home).last()
					    : $('.home.active').prev($home),
                sectionfade = $('.home.active'),
                headingIn = $(sectionIn).children().children('.heading'),
                headingfade = $('.heading.active'),
                indicatorIn =					$('.indication.active').prev($indi).length === 0
    ...
    $('.container').on(
        'wheel',
        _.debounce(scrolling, 200, { leading: true, trailing: false }),
    );
    ...
    const clickImg = j => () => {
        ...
        const thumbImg = $(`.thumbimg${j}`),
            ...
            imgtext1 = $(
                `.detail${j} > .imgContainer > .pitext > .project-i-text-1 > div`,
            ),
            imgtext2 = $(
                `.detail${j} > .imgContainer > .pitext > .project-i-text-2 > div`,
            ),
    ...
    function slideshowbutton(j) {
        const nextslide =			$(`.pi${j}-div.active`).next(`.pi${j}-div`).length === 0
    ...
    $('.leftarrow').click(() => {
        ...
        $('.allSkillsContainer').animate(
            { scrollLeft: `-=${getclientwidth()}` },
            400,
        );
    });
    ...
    gets1Timeline = () => {
        ...
        s1.set('.html5body', { xPercent: -50 })
            ...
            .fromTo(
                ...
                0.5,
                { scale: 1 },
                { ease: Power4.easeIn, scale: 1.1, zIndex: 100 },
            )
    ...
    gets2Timeline = () => {
        ...
        s2.fromTo(
            ...
            .fromTo(
                ...
                0.25,
                { fontWeight: '200' },
                { fontFamily: 'Dancing Script', fontWeight: '900', color: '#0298CC' },
            )
            .fromTo(
                '.css3',
                0.25,
    ...
    gets3Timeline = () => {
        ...
        s3.fromTo(
            ...
            .to(
                ...
                3,
                {
                    text: { value: randomText, oldClass: 'css1', newClass: 'js1' },
                    ease: Power1.easeIn,
                },
                '#line1',
            )
            ...
            .to(
                ...
                1.5,
                {
                    text: { value: `  Its   ${currentDay}  `, newClass: 'jsbig' },
                    ease: Power1.easeIn,
                },
                '#line4',
            )
    ...
    gets4Timeline = () => {
        ...
        s4.fromTo(
            ...
            .set('.cssh2', {
                text: {
                    value:
						' import  <span class="reacth1">  React, { Component }  </span>  from  <span class="reacth1">  "react" </span>; <br>  import  <span class="reacth1">  SkillsContent </span> from <span class="reacth1">" ./SkillsContent " </span>; ',
                },
                ease: Power1.easeOut,
            })
            ...
            .set(
                '.css1, .css2, .css3, .css4',
                { text: '   <br>', ease: Power1.easeIn },
                '-=1',
            )
            .set('.css1', {
                text: {
                    value:
						' render() { <br>  return ( <br> <span class="react2"> < div className = "skillsContentGrid" > </span> <br> ',
                },
                ease: Power1.easeIn,
            })
            ...
            .set('.css4js', {
                text: {
                    value:
						'<span class="react3"> < <b>SkillsContent</b> content = { this.state.currentContent } /> </span>',
                    oldClass: 'css4js',
                    newClass: 'react1',
                },
                ease: Power1.easeIn,
            ...
            .set('.css2', {
                text: { value: ' <span class="react2"> <br> < / div > </span> <br>' },
                ease: Power1.easeIn,
            })
    ...
    gets6Timeline = () => {
        ...
        // .fromTo('.nonhtml5', 0.5, {margin:'0% 0% 0% 0%'}, {margin:'0 0 0 10%'})
            ...
            .to('.bs1', 0.01, {
                text: { value: '<i>< h2 class = "text-center font-weight-bold" ></i>' },
            })
    ...
    gets7Timeline = () => {
        ...
        s7.fromTo(
            ...
            .to(
                '.css4n',
                2,
                { text: { value: 'Look at console for details' }, ease: Power1.easeIn },
                '+=2',
            );
        return s7;
    };
    ...
    const tap = new TimelineMax({ paused: true, repeat: -1 })
        ...
        .set(
            '.backgroundWrapper',
            { backgroundImage: 'url(./assets/aboutImages/2.jpg)' },
            '-=1',
        )
        ...
        .set(
            '.aboutWrapper',
            { backgroundImage: 'url(./assets/aboutImages/3.jpg)' },
            '-=1',
        )
        ...
        .set(
            '.backgroundWrapper',
            { backgroundImage: 'url(./assets/aboutImages/4.jpg)' },
            '-=1',
        )
        .to('.backgroundWrapper', 0.5, { autoAlpha: 1 })
        .fromTo(
        ...
        .set(
            '.aboutWrapper',
            { backgroundImage: 'url(./assets/aboutImages/5.jpg)' },
            '-=1',
        )
        .to('.backgroundWrapper', 0.5, { autoAlpha: 0 })
        .fromTo(
        ...
        .set(
            '.backgroundWrapper',
            { backgroundImage: 'url(./assets/aboutImages/6.jpg)' },
            '-=1',
        )
        .to('.backgroundWrapper', 0.5, { autoAlpha: 1 })
        .fromTo(
        ...
        .set(
            '.aboutWrapper',
            { backgroundImage: 'url(./assets/aboutImages/1.jpg)' },
            '+=1',
        )
        .to('.backgroundWrapper', 0.5, { autoAlpha: 0 });
